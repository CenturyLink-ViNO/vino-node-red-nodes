...
if (!ctl.ui.renderUI)
{
   ctl.ui.renderUI = function (node, commands, commonParameters)
   {
      ...
      let baseTypes = JSON.parse(JSON.stringify(commands));
      let baseTypeSelect = jQuery('#baseType-select');
      let statusOptionsButton = jQuery('#statusOptions-button');
      if (!node.baseTypes || node.baseTypes.length < 1)
      {
         node.baseTypes = baseTypes;
      }
      node.baseTypes = ctl.ui.mergeSavedDataWithConfiguration(baseTypes, node.baseTypes, commonParameters);
      let categorizedBaseTypes = ctl.ui.categorizeBaseTypes(node.baseTypes);
      ...
      node.inputTable = ctl.ui.buildInputTable(domInputTable, { data: [] }, inputParameterDialog, node.baseTypes);
      node.outputTable = ctl.ui.buildOutputTable(domOutputTable, { data: [] }, outputParameterDialog, node.baseTypes);
...
if (!ctl.ui.selectBaseType)
{
   ...
   {
      for (let baseTypeIndex in node.baseTypes)
      {
         let baseType = node.baseTypes[baseTypeIndex];
         ...
         let mergeOutputParameters = function (configParametersMap, activeParametersMap)
         {
            ...
            while (!iter.done)
            {
               ...
               if (activeParametersMap.has(key))
               {
                  ...
                  // merging output details would go here, but generally speaking we probably want to use the config data here?
                  //target.outputDetails.type = source.outputDetails.type;
                  //target.outputDetails.format = source.outputDetails.type;
...
if (!ctl.ui.editInputParameter)
{
   ctl.ui.editInputParameter = function (table, parameter, inputParameterDialog)
   {
      ...
      inputParameterDialog.data('table', table).data('new', false).data('userAdded', parameter.userAdded || false).dialog('open');
...
if (!ctl.ui.getOutputParams)
{
   ctl.ui.getOutputParams = function (node)
   {
      ...
      if (node.baseTypes)
      {
         for (let baseTypeIndex in node.baseTypes)
         {
            if (node.baseTypes.hasOwnProperty(baseTypeIndex))
            {
               let baseType = node.baseTypes[baseTypeIndex];
...
if (!ctl.ui.save)
{
   ctl.ui.save = function (node)
   {
      ...
      for (let i = 0; i < node.baseTypes.length; i = i + 1)
      {
         if (node.baseTypes[i].key === selectedBaseType)
         {
            node.selectedBaseType = selectedBaseType;
            if (inputParameters)
            {
               node.baseTypes[i].inputParameters = JSON.parse(JSON.stringify(inputParameters));
            }
            if (outputParameters)
            {
               node.baseTypes[i].outputParameters = JSON.parse(JSON.stringify(outputParameters));
            }
            break;
         }
      }
