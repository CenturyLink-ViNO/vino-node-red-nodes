/*
 * users.js: Client methods for working with users on database within instances from Openstack Trove.
 *
...
exports.createUser = function createUser(options, callback) {
  ...
  function assessOptions(opts) {
    ...
    ['username', 'password', 'database', 'instance'].forEach(function (required) {
      // API allows for individual or multiple databases
      if (!opts[required] && ( required === 'database' && !opts['databases'])) {
    ...
    if (opts && opts['database'] && typeof opts['database'] === 'string') {
      databases.push({ 'name' : opts['database']});
    }

    if (opts && opts['database'] && opts['database'] instanceof Database) {
      databases.push({ 'name' : opts['database'].name});
    }
