...
_convertNetworkToWireFormat = function (details){
  ...
  wireFormat.admin_state_up = details.admin_state_up === true || details.adminStateUp === true;
  wireFormat.name = details.name;
  wireFormat.shared = details.shared;
  wireFormat.tenant_id = details.tenantId || details.tenant_id;
  wireFormat.dns_domain = details.dnsDomain || details.dns_domain;
  wireFormat.mtu = details.mtu;
  wireFormat.port_security_enabled = details.portSecurityEnabled === true || details.port_security_enabled === true;
  wireFormat.project_id = details.projectId || details.project_id ;
  wireFormat['provider:network_type'] = details.providerNetworkType || details['provider:network_type'];
  wireFormat['provider:physical_network'] = details.providerPhysicalNetwork || details['provider:physical_network'];
  wireFormat['provider:segmentation_id'] = details.providerSegmentationId || details['provider:segmentation_id'];
  wireFormat.qos_policy_id = details.qosPolicyId || details.qos_policy_id;
  wireFormat['router:external'] = details.routerExternal || details['router:external'];
  wireFormat.segments = details.segments;
  wireFormat.vlan_transparent = details.vlanTransparent === true || details.vlan_transparent === true || undefined;
  wireFormat.description = details.description;
  wireFormat.is_default = details.isDefault === true || details.is_default === true || undefined;
  wireFormat.availability_zone_hints = details.availabilityZoneHints || details.availability_zone_hints;
