...
Client.prototype.rpc = function (request, callback) {
...
Client.prototype.send = function (xml, messageID, callback) {
   const self = this;
...
Client.prototype.parse = function (xml, callback) {
     const self = this;
     xml2js.parseString(xml, this.parseOpts, function checkRPCErrors(err, message) {
        ...
        catch (error)
        {
           ...
           const errorMessage = `There was an error while checking for RPC errors:\n\n${error}\n\nReceived message:\n\n${messageString}`;
...
Client.prototype.open = function (callback) {
         ...
         vasync.waterfall([
               function getStream(next) {
                  ...
                  catch (error)
                  {
                     ...
                     self.emit('error', `There was an error setting the SSH subsystem to netconf: ${error}`);
                  }
               },
   ...
   {
      self.emit('error', `There was an error while attempting to connect to the netconf server: ${error}`);
   }
};
