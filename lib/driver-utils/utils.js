...
module.exports = {
    ...
    processHandlebarsTemplate: function(template, inputParameters) {
    ...
    processMustacheTemplate: function(template, inputParameters) {
        function createContextFromInputParameters() {
            let ctx = {};
            for (let idx in inputParameters)
            {
    ...
    parseResponseForOutputParameters: function(outputParams, response) {
        function processXPath(path, content) {
            ...
            if (outputParams.hasOwnProperty(idx)) {
                ...
                if (newParam.outputDetails && newParam.outputDetails.type) {
                    switch (newParam.outputDetails.type) {
                        case 'XPATH':
                            newParam.setValue(processXPath(newParam.outputDetails.format, response));
                            break;
                        case 'JSONPATH':
